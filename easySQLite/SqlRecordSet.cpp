...
int RecordSet::on_next_record(void *param, int column_count, char **values,
                              char **columns)
{
...
Record *RecordSet::getRecordByPrimaryKey(const char *keyName, int keyValue)
{
    for (int record_index = 0; record_index < count(); record_index++) {
        if (Record *record = getRecord(record_index))   {
            if ((int)record->getValue(keyName)->asInteger() == keyValue) {
                return record;
            }
        }
    }
