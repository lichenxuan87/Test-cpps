...
void Record::setnullptr(int index)
{
...
void Record::setnullptr(string fieldName)
{
    if (Field *field = fieldByName(fieldName)) {
        setnullptr(field->getIndex());
    }
}
...
string Record::toSqlUpdate(string tableName, unsigned int updateKeyIndex)
{
    ...
    if (updateKeyIndex < (unsigned int)_fields->count()) {
        Field *field = _fields->getByIndex(updateKeyIndex);
        Value *value = getValue(field->getName());
        s += " where "
             + field->getName()
             + " = "
             + value->toSql(field->getType());
    }
