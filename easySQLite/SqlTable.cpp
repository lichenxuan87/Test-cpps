...
{
    const string sql = "delete from " + _tableName + (whereCondition.empty() ? "" :
...
{
    const string queryStr = "select * from " + _tableName + " where _ID = " +
                            intToStr(keyId);
...
Record *Table::getRecordFromCacheByPrimaryKey(const char *primaryKey,
        int keyValue)
{
    if (_allRecordsCached) {
        return _recordset.getRecordByPrimaryKey(primaryKey, keyValue);
    }
...
// Return number of rows being modified
int Table::updateRecord(Record *record, unsigned int fieldIndex)
{
    if (record) {
        const string sql = record->toSqlUpdate(name(), fieldIndex);

        RecordSet rs(_db, _recordset.fields());

        if (rs.query(sql)) {
            return sqlite3_changes(_db); // TBD: this might have multi-thread issue
        }
    }
    return -1;
}
...
Table *Table::createFromDefinition(sqlite3 *db, string tableName,
